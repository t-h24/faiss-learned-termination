                  ...
                  VisitedTable& vt) const
{
  if (upper_beam == 1) {
    ...
    if (search_bounded_queue) {
      ...
      search_from_candidates(qdis, k, I, D, distance_comps, candidates, vt, 0);
    } else {
      std::priority_queue<Node> top_candidates =
    ...
    for(int level = max_level; level >= 0; level--) {
      ...
      if (level == 0) {
        nres = search_from_candidates(qdis, k, I, D, distance_comps, candidates, vt, 0);
      } else  {
        nres = search_from_candidates(
          qdis, candidates_size,
          I_to_next.data(), D_to_next.data(), NULL,
          candidates, vt, level
        );
      }
      vt.advance();
